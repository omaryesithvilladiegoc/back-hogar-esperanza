...
export class PostService {
  ...
  async create(createPostDto: CreatePostDto) {
    ...
    try {
      const userFound: User = await this.userRepository.findOneBy({
        id: idUser,
      });
    ...
    try {
      ...
      if (!posts) {
        throw new BadRequestException(
          'Hubo un error al obtener los posts o no hay posts disponibles',
        );
      }
  ...
  async findOne(id: string) {
    try {
      ...
      if (!postFound)
        throw new BadRequestException('Hubo un error al obtener el post');
      return postFound;
    } catch (error) {
      throw error;
    }
  ...
  async getPostById(id: string): Promise<Post> {
    try {
      ...
      if (!postFound)
        throw new BadRequestException('HUbo un error al encontrar el post');
      return postFound;
    } catch (error) {
      throw error.message;
    }
  ...
  async update(id: string, updatePostDto: UpdatePostDto) {
    try {
      ...
      if (!updatedPost)
        throw new BadRequestException('Hubo un error al actualizar el post');
  ...
  async remove(id: string) {
    try {
      ...
      if (!response)
        throw new BadRequestException('HUbo un error al eliminar el post');
      return response;
    } catch (error) {
      throw error;
    }
  ...
  async updateSize(size: number, idPost: string) {
    try {
      ...
      const postUpdated = await this.postRepository.update(
        { id: idPost },
        { size },
      );
      if (!postUpdated)
        throw new BadRequestException('Hubo un error actualizando el post');
